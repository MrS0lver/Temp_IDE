...
class SimpleIDE:
    ...
    def run_code_thread(self):
        ...
        try:
            ...
            user_input = None
            if "cin" in code:  # Check if the code has cin (or any other input)
                user_input = self.prompt_user_input()

            # Run the executable and pass input
            ...
            # Pass input to the process if any
            if user_input:
                output, errors = result.communicate(input=user_input)
            else:
    ...
    def prompt_user_input(self):
        ...
        input_label = tk.Label(input_window, text="Enter input for the program:", font=("Arial", 10))
        input_label.pack(pady=10)

        input_text = tk.Text(input_window, height=5, width=40, font=("Courier New", 12))
        input_text.pack(pady=5)

        input_data = tk.StringVar()

        def submit_input():
            input_data.set(input_text.get("1.0", tk.END))
            input_window.destroy()

        submit_button = tk.Button(input_window, text="Submit", command=submit_input, bg="#61afef", fg="white")
        submit_button.pack(pady=10)

        input_window.grab_set()  # Lock focus to the input window
        self.root.wait_window(input_window)  # Wait until input window is closed

        return input_data.get().strip()  # Return the user input as a string
